...
DIMESHIFT_DIR="/home/vagrant/workspace/fse2019/dimeshift"
CONSERVED_DIR="$DESKTOP_DIR/dimeshift-conserved-extra-5"
MAX_RUNS=5
...
while [ "$LAST_NUM" -lt "$MAX_RUNS" ]; do
    ...
    while :; do
        ...
        if [[ -d "$DESKTOP_DIR/testdimeshiftAdaptiveSequence_0" ]] && \
    ...
    # Move directory
    mv "$DESKTOP_DIR/testdimeshiftAdaptiveSequence_0" "$CONSERVED_DIR/$NEXT_NUM/" || {
