#!/bin/bash
# ===== CONFIG =====
DESKTOP_DIR="/home/vagrant-old/Desktop"
DIMESHIFT_DIR="/home/vagrant-old/workspace/fse2019/dimeshift"
CONSERVED_DIR="$DESKTOP_DIR/dimeshift-baseline-extra-5"
MAX_RUNS=5
SLEEP_INTERVAL=60  # Check every 1 minute for long runs
# ==================
...
while [ "$LAST_NUM" -lt "$MAX_RUNS" ]; do
    ...
    # --- Run Experiment ---
    cd "$DIMESHIFT_DIR" || { echo "[ERROR] Failed to cd to $DIMESHIFT_DIR"; exit 1; }
    ./runExp.sh 1 DIGSI APOGEN 1800 || {
    ...
    while :; do
        ...
        if [[ -d "$DESKTOP_DIR/testdimeshiftAdaptiveComplete_0" ]]; then
            break
        fi
    ...
    # Move directory
    mv "$DESKTOP_DIR/testdimeshiftAdaptiveComplete_0" "$CONSERVED_DIR/$NEXT_NUM/" || {
