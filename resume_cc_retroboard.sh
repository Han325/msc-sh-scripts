...
# ===== CONFIG =====
SOURCE_DIR="/home/vagrant-code-coverage/Desktop/retroboard-baseline-15-run"
RESULTS_DIR="/home/vagrant-code-coverage/workspace/test-generation-results/resultsRetroboard"
CODECOVERAGE_DIR="/home/vagrant-code-coverage/workspace/codecoverage"
FINAL_RESULTS_DIR="/home/vagrant-code-coverage/Desktop/retroboard-15-run-RESULTS"
DESKTOP_DIR="/home/vagrant-code-coverage/Desktop"
MAX_RUNS=15
RESUME_FROM=${1:-7}  # Default to run 3, or take first argument
# ==================

echo "===== RESUMING RETROBOARD ANALYSIS FROM RUN $RESUME_FROM ====="

echo "[$(date)] Skipping Phase 1 (file copying) - assuming already completed"

# Phase 2-6: Process each run individually starting from RESUME_FROM
for run in $(seq $RESUME_FROM $MAX_RUNS); do
    ...
    while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
        ...
        ./run.sh retroboard \
                 "/home/vagrant-code-coverage/workspace/test-generation-results/resultsRetroboard/$run" \
    ...
    echo "[DEBUG] Running: ./calculate-auc.sh \"/home/vagrant-code-coverage/workspace/test-generation-results/resultsRetroboard/$run/testretroboardLLM_0\""
    ./calculate-auc.sh "/home/vagrant-code-coverage/workspace/test-generation-results/resultsRetroboard/$run/testretroboardLLM_0" > "/home/vagrant-code-coverage/Desktop/retroboard-15-run-RESULTS/$run/results-auc.txt" 2>&1 || {
        echo "[ERROR] AUC calculation failed for run $run";
        echo "[DEBUG] Contents of results-auc.txt:";
        cat "/home/vagrant-code-coverage/Desktop/retroboard-15-run-RESULTS/$run/results-auc.txt" 2>/dev/null || echo "No results-auc.txt file found";
        exit 1;
    }
    ...
    python3 fault-auc.py > "/home/vagrant-code-coverage/Desktop/retroboard-15-run-RESULTS/$run/fault-auc.txt" || {
...
echo "===== RUNS $RESUME_FROM-$MAX_RUNS COMPLETED SUCCESSFULLY ====="
echo "[$(date)] Resume analysis finished!"
