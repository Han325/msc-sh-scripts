...
# ===== CONFIG =====
SOURCE_DIR="/home/vagrant-cc-enhanced/Desktop/dimeshift-baseline-15-run"
...
FINAL_RESULTS_DIR="/home/vagrant-cc-enhanced/Desktop/dimeshift-15-run-RESULTS"
DESKTOP_DIR="/home/vagrant-cc-enhanced/Desktop"
MAX_RUNS=15
RESUME_FROM=${1:-7}  # Default to run 3, or take first argument
# ==================

echo "===== RESUMING DIMESHIFT ANALYSIS FROM RUN $RESUME_FROM ====="
...
for run in $(seq $RESUME_FROM $MAX_RUNS); do
    ...
    ./calculate-auc.sh "/home/vagrant-cc-enhanced/workspace/test-generation-results/resultsDimeshift/$run/testdimeshiftLLM_0" > "/home/vagrant-cc-enhanced/Desktop/dimeshift-15-run-RESULTS/$run/results-auc.txt" 2>&1 || {
        echo "[ERROR] AUC calculation failed for run $run";
        echo "[DEBUG] Contents of results-auc.txt:";
        cat "/home/vagrant-cc-enhanced/Desktop/dimeshift-15-run-RESULTS/$run/results-auc.txt" 2>/dev/null || echo "No results-auc.txt file found";
        exit 1;
    }
    ...
    python3 fault-auc.py > "/home/vagrant-cc-enhanced/Desktop/dimeshift-15-run-RESULTS/$run/fault-auc.txt" || {
